<?php
class td_block_author_vdm extends td_block {
    ...
    function render($atts, $content = null) {
        ...
        $buffy = '';
        $buffy .= '<div class="' . $this->get_block_classes() . (!empty($block_width) ? '" style="max-width:'. $block_width . '" ' : '" ') . $this->get_block_html_atts() . '>';
        ...
        if(!empty($author_info_text)) {
            $buffy .= $author_info_text;
        }else{
            $buffy .= $td_author->description;
        }
        $buffy .= '</div>';

        if(!empty($td_author->user_url) && strrpos($td_author->user_url, 'viedemotard.fr') === false) {
            $buffy .= '<div class="td-author-url">';
        ...
        if(!empty($author_url_text)) {
            ...
            if(!empty($author_url)) {
             $buffy .= '<a href="' . $author_url . '"' . $td_target . '>' . $author_url_text . '</a>';
                ...
                $buffy .= '<a href="/author/' . $td_author->user_nicename . '"' . $td_target . '>' . $author_url_text . '</a>';
            }
            $buffy .= '</div>';
        }
